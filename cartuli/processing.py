...
def inpaint(image: Image.Image, inpaint_size: int = 16, image_crop: int = 2, corner_radius: int = 12) -> Image.Image:
    ...
    expanded_image = ImageOps.expand(image, border=inpaint_size, fill='white')

    mask_image = Image.new('L', tuple(s + inpaint_size*2 for s in image.size), color='white')
    ...
    mask_image_draw.rounded_rectangle(
        (inpaint_size + image_crop, inpaint_size + image_crop,
         mask_image.size[0] - inpaint_size - image_crop,
         mask_image.size[1] - inpaint_size - image_crop),
